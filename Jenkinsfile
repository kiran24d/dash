    ...
    stage ('Find changed files') {
        ...
        steps {
          script {
              ...
              supported_files = []
              ignored_files = []
              filesChanged.findAll { file -> fileExists(file) && (file.endsWith('.yaml') || file.endsWith('.yml') || file.endsWith('.json')) }.each {
                ...
                timeout(time: 10, unit: 'MINUTES'){
                  def result = build(job: "aws-update-cloudformation-stack",
                        parameters: [
                            ...
                            string(name: 'wf_cloud', value: 'Shared Technology Group'),
                            string(name: 'wf_business_unit', value: 'Infrastructure'),
                            string(name: 'wf_product', value: 'Security'),
